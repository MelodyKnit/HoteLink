...
1. 先确定业务模块和数据库模型。
2. 初始化 Django 项目与基础 app。
3. 初始化前端 Monorepo，建立 `user-web` 和 `admin-web`。
4. 先完成认证、房态、订单三个主链路。
5. 再补支付、报表、会员、通知等增强模块。
...
1. 生成一份更完整的 `docs/architecture.md` 技术架构文档。
2. 直接初始化 `backend` 的 Django 项目目录规划。
3. 直接初始化 `frontend` 的 Vite + Vue3 + TS Monorepo 结构。
4. 先帮你设计酒店管理系统的数据库表结构。
