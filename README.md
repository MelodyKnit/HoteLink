...
- 已支持多 AI 供应商配置与切换：可在管理端新增多个模型供应商，并切换当前活跃供应商。
- 新增管理端系统重置能力：系统管理员可执行一键重置，清除业务数据并恢复初始状态（保留管理员账号）。
- 新增随机数据脚本：可批量生成用户、酒店、房型、订单、评价，便于功能联调和回归测试。
- 已补充后端单元测试：覆盖 AI 供应商配置、系统重置等关键流程。
- 已执行基础低级错误检查：后端编译、后端测试、前端 TypeScript 类型检查。
