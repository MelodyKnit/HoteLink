...
1. ✅ 业务模块与数据库模型（17 个 Model，跨 7 个 app）
2. ✅ 认证、房态、订单核心链路
3. ✅ 支付、报表、会员、通知、优惠券、发票、评价等增强模块
4. ✅ AI 多供应商接入与智能客服
5. ✅ Docker 开发 + 生产环境完整编排

下一阶段重点：

- 管理端前台业务页面（入住办理、退房结算、续住/换房）
- 管理端财务模块页面
- 权限与审计日志页面
- 详见 [feature-improvements.md](docs/feature-improvements.md)
...
- **样式**（`packages/styles`）：Tailwind + Less 主题
...
当前后端已注册 **88 个 API 端点**，定义 **17 个 Django Model**（跨 7 个业务 app），核心能力包括：

- **认证**：JWT 注册、登录、管理员登录、刷新令牌、当前用户信息
- **公共查询**：首页、酒店列表、搜索建议、酒店详情、评价列表、房型价格日历、城市、字典
- **用户中心**：资料维护、头像上传、密码修改、收藏、订单（创建/支付/取消）、评价、优惠券（领取/使用）、发票申请、积分、消息通知
- **管理端**：经营概览与趋势图表、酒店 CRUD、房型 CRUD、库存管理、订单处理（含入住/退房）、评价回复、用户管理、员工管理、系统配置、系统重置
- **AI 辅助**：用户端智能客服（多轮对话 + 流式输出 + 预订引导）、管理端经营摘要/评价摘要/回复建议（壳接口）、AI 供应商管理（5 家预设）

后续可继续补充：

- WebSocket 实时通知
- 更完整的权限和酒店维度隔离
- Celery 异步任务（已配置，未定义实际 task）
- 管理端 AI 接口接入真实 LLM 调用
