...
HoteLink 的 AI 不是前端直连模型，而是“前端 -> Django API -> AI 服务层 -> 多供应商模型”的后端统一接入模式：

- 用户端支持普通聊天、流式聊天、订房编排、推荐和酒店对比
- 管理端支持摘要、定价、经营报告、营销文案、情感分析和异常分析
- 所有 AI 密钥都只在后端保存，前端拿不到明文
- AI 只负责“建议、解释、生成”，不直接替代支付、退款、库存和订单规则

### 1.2 这份文档适合什么时候看

- 你要维护 AI 聊天、流式输出、Prompt、模型供应商切换
- 你想确认当前 AI 已经落地了哪些能力，哪些还只是规划
- 你要排查“为什么模型能回答这个问题”或“为什么这里不能让 AI 直接执行”

如果你只是想知道接口路径，优先看 [`api-spec.md`](./api-spec.md) 或 [`api-inventory.md`](./api-inventory.md)。
