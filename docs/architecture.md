...
已实现用户下单、支付、取消，以及管理端改状态、办理入住、办理退房、续住/换房与未入住处理。订单生命周期区分 `paid/confirmed`、`checked_in`、`completed`、`no_show` 等状态：未办理入住且生命周期已结束的已支付订单会进入 `no_show`，不会被错误保留为“已支付”或绕过入住流程直接完结。
