...
- 要有营销感，也要有决策信息
- 房型列表需要直接承接预订动作
- 若从订单详情页携带 `from=order_detail&order_id=...` 进入，且酒店当前已下线，则切换为历史订单关联展示模式：保留基础信息与返回订单入口，不展示新的可售房型。
...
- 订单状态包含 `未入住(no_show)` 独立标签；当后端返回 `lifecycle_warning` 时，列表以近期变更/异常提示展示，避免用户继续看到过期的“已支付/已确认”状态。
...
- 详情页顶部展示订单状态、订单号、实付金额、入住/离店日期和晚数，降低用户核对成本。
- 已完成订单展示“发票服务”卡片和“开发票/查看发票”操作，弹窗内可选择已有发票抬头并提交开票申请。
- “点击查看酒店”与“再次预订”入口会携带当前 `order_id` 跳转酒店详情，确保历史订单关联酒店在下线后仍可被本人查看。
- `未入住` 订单在状态时间线与变更轨迹中明确展示 `no_show_at` 和生命周期提示；入住日已开始的已支付/已确认订单不再展示用户自助取消入口，引导联系酒店处理。
...
- 顶部菜单提供移动端友好的“聊天历史”抽屉：按最近使用排序展示本地最近 20 条摘要、消息数，并提供始终可见的恢复/删除操作，避免依赖 hover 才能完成历史管理。
...
> | 34 | AI 客服辅助页 | — | — | 📐 设计中 |
> | — | 系统状态页 | `/admin/system-status` | `SystemStatusView.vue` | ✅ 已实现（文档未列设计） |
...
- 订单详情抽屉
- 批量操作
- `未入住(no_show)` 筛选、异常提示与“标记未入住”快捷操作；入住日未到时不展示办理入住入口，退房/续住/换房入口仅对已入住订单展示
...
- 支付记录
- 退款记录
- 生命周期告警与状态时间线，包含 `未入住`、`no_show_at` 与操作备注轨迹
- 操作日志
...
- 支持入住日期筛选，仅展示当日可办理入住的订单
- 入住操作前校验订单资格（已支付/已确认、入住日已开始、未超过离店日、未被标记未入住），不符合条件的订单不可勾选
...
- 系统配置页当前保留平台基础配置与系统重置，并提供跳转到“支付网关”“AI 配置”的快捷入口。
...
- 风格需与系统设置、AI 配置页保持一致，适合高频运维配置
- 网关状态必须明确区分“已启用”“沙箱联调”“待补齐参数”
- 保存后应直接影响用户端支付页的方式展示与支付动作协议
...
- 过程轨迹：当 `meta.agent_state` 存在时，消息顶部先展示轻量“分析中 / 已完成分析”轨迹；默认收起为小字号灰色状态条，展开后展示摘要、依据、指标、阶段说明与安全边界，不展示系统提示词或原始内部推理
- 轨迹排版：过程轨迹保持灰阶、小字号和低视觉重量；前端会限制依据、指标和阶段明细数量，避免把解释区做成大段日志或装饰性提示词展示
- 轨迹依据：`agent_state.facts[]` 用于展示本轮真实命中的城市、酒店、订单、筛选条件、推荐依据和动作入口；`agent_state.metrics[]` 用于展示候选酒店数、房型数、入口数、预算、评分、依据条数等短指标，帮助用户判断 AI 是否真正结合了系统数据
- 订房交互：支持在聊天消息中以轻量“推荐下一步”动作区渲染城市、酒店、房型入口，点击后继续对话、查看酒店或直接跳转 `/booking`
- 酒店候选：订房助手可在动作卡片中展示 `badges` 和 `highlights`，例如参考交通点直线距离、标签/地址命中、数据不足说明；这些内容使用小字号灰阶说明与低饱和标签，不抢占主回复。
- 客服交互：客服场景支持结构化快捷动作（订单详情、去支付、取消引导、发票、通知、帮助中心、切换订房助手），动作卡片使用短标签、低饱和边框和明确确认标识，避免与主回答争抢注意力
...
- 问题续聊：跨助手跳转时通过 `query.ask` 透传原问题，目标页面自动发送该问题并移除一次性参数；首页 AI 入口使用 `ask_mode=new_only`，仅在当前模式仍是新聊天时自动发送引导问题，已有本地会话时只进入页面不再追加消息
